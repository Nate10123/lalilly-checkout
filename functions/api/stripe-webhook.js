...
export async function onRequestPost(context) {
  ...
  try {
    ...
    const items = cart.map(({ id, qty, size, color }) => {
    ...
    // Save this order so the customer can look up its status/tracking
    // later on the Track Your Order page, and so the Printful shipping
    // webhook (printful-webhook.js) has somewhere to write the tracking
    // number once it comes in. ORDERS is a KV namespace — see wrangler.toml.
    if (env.ORDERS && pfData.result && pfData.result.id) {
      const pfOrderId = String(pfData.result.id);
      const orderRecord = {
      ...
      await env.ORDERS.put(`pforder:${pfOrderId}`, JSON.stringify(orderRecord));
      await env.ORDERS.put(`session:${session.id}`, pfOrderId);
    } else if (!env.ORDERS) {
