...
import { PRODUCTS, getPriceCents, hasSizes, hasColors } from '../_shared/products.js';

// Matches the "Free shipping over $90" banner on the storefront.
const FREE_SHIPPING_THRESHOLD_CENTS = 9000;
...
export async function onRequestPost(context) {
  ...
  try {
    ...
    // Normalize + validate quantities, colors, and sizes, using OUR price
    // list and OUR product definitions, never the browser's.
    const normalizedCart = cart.map((item) => {
      ...
      if (needsColor && (!item.color || !product.colors.includes(item.color))) {
        throw new Error(`Please choose a color for "${product.name}".`);
      }

      if (needsSize) {
        const validSizes = needsColor
          ? Object.keys(product.variantsByColor[item.color] || {})
